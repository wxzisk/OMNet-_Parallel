//
// Generated file, do not edit! Created by nedtool 5.4 from fat_tree.msg.
//
...
FatTreeMsg::FatTreeMsg(const char *name, short kind) : ::omnetpp::cMessage(name,kind)
{
    this->src_ppid = 0;
    this->dst_ppid = 0;
    this->hopCount = 0;
...
const char *FatTreeMsgDescriptor::getFieldDynamicTypeString(void *object, int field, int i) const
{
    omnetpp::cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount())
